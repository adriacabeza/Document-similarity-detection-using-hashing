...
To make it easier for us to compute and visualize graphically the results we have set up a python script. Although to generate said information first we have to execute the corresponding programs (posar lo del makefile i com es guarda). When we execute the script we are prompted with a series of options to choose from, obviously if we have not done the experiment chosen beforehand we wont have any data to read and thus we wont have a graphic or we will get a graphing made from previous data.
\clearpage
\section{Conclusions}
