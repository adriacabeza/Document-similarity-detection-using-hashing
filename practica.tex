...
In the following experiment we wanted to test how precise the outputed similarity for the Jaccard Similarity Approximation is.
...
\bigskip
As we expected when the number of hash functions is small the difference between the real similarity and the approximation differs a lot. Moreover, as the number of hash functions increases the difference of similarity decreases significantly.  The reason we believe this happens is %FICAR AQUÍ TONI
...
\item Larger number of hash functions result in a better precision for the Jaccard Similarity Approximation.
