...
And, on the other hand, for values of $k$ that span more than a word, the similarity between documents is very small (because the documents don't share the word order, so the probability of identical k-shingles decreases).
...
We have tested different k sizes for our k-shingles and three different hash functions for fillinf the signature matrix explained (FICAR SECTION) to check their different performance. 

Our hash functions are MurmurHash3, Multiplicative Hash and Modular Hash and are explained in the section 8.

As we expected, the time increases as the size of the k-shingle increases in a general basis. This behaviour can be explained in the number of different shingles we would have to work with.
